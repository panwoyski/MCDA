...
#def utastar(performanceTable, criteriaMinMax, criteriaNumberOfBreakPoints, alternativesRanks, epsilon):
    ...
    #META_UTA TECHIQUE - STEP 4 IN UTASTAR

    if (lpsolution['fun'] != 0):
        mat = np.vstack((mat,obj))
        dirr = np.append(dirr, "<=")
        rhs = np.append(rhs, lpsolution['fun'] + epsilon)

        maxSigma = 0
        ind = len(obj)-(2*numAlt)

        for i in range(len(obj)-(2*numAlt), len(obj)):
            if lpValues[i] > maxSigma:
                maxSigma = lpValues[i]
                ind = i
                obj[i] = 0
